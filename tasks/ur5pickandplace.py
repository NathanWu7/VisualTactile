...
class Ur5pickandplace(BaseTask):
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        # Set control limits
        self.cmd_limit = to_torch([0.01, 0.01, 0.01, 0.05, 0.05, 0.05, 0.1], device=self.device).unsqueeze(0)
        ...
        table_pos = [0.10, 0.15, self.table_stand_height/2]
        table_opts = gymapi.AssetOptions()
        table_opts.fix_base_link = True
        table_asset = self.gym.create_box(self.sim, *[0.4, 0.8, self.table_stand_height], table_opts)
        ...
        barrier_height = 0.15
        barrier_pos = [0.65, 0.00, self.table_stand_height/2]
        barrier_opts = gymapi.AssetOptions()
        barrier_opts.fix_base_link = True
        ...
        barrier_pose.p = gymapi.Vec3(*barrier_pos)
        barrier_pose.r = gymapi.Quat(0.0, 0.0, 0.0, 1.0)
        ...
        self.max_agg_bodies = num_robot_bodies + 4     # 1 for table, table stand, barrier, cubeA
        max_agg_shapes = num_robot_shapes + 4          # 1 for table, table stand, barrier, cubeA
        ...
        for i in range(self.num_envs):
            ...
            barrier_actor = self.gym.create_actor(env_ptr, barrier_asset, barrier_pose, "barrier", i, 0, 0)
            #table_actor1 = self.gym.create_actor(env_ptr, table_asset, table_pose, "table1", i, 1, 0)
    ...
    def init_data(self):
        ...
        self._init_cubeA_state[:, :3] = torch.tensor([0.30, 0.135, self.table_stand_height + self.cubeA_size / 2], device=self.device)
        self._init_cubeA_state[:, 6] = torch.tensor([1], device=self.device)

        self.init_goal_pos[:,:3] = to_torch([0.70, 0.00, self.table_stand_height + 0.2], device=self.device)
    ...
    def _update_states(self):
        self.states.update({
            ...
            "cube_to_goal": self.goal_pos[:, :3] - self._cubeA_state[:, :3], 
    ...
    def compute_contact_force(self):
        net_contact_force = gymtorch.wrap_tensor(self.net_contact_force_tensor).view(self.num_envs, self.max_agg_bodies - 1 , 3)
        # 计算总接触力
    ...
    def _random_cubeA_state(self, _init_cubeA_state, env_ids):
        ...
        centered_cube_xy_state = torch.tensor([0.15, 0.4, 0.83], device=self.device, dtype=torch.float32)
        _init_cubeA_state[env_ids, 2] = self.table_stand_height + self.cubeA_size / 2
        _init_cubeA_state[env_ids, :2] = centered_cube_xy_state[0:2] + 1.0 * self.start_position_noise * \
    ...
    def _random_goal_state(self, init_goal_state):
        centered_goal_xy_state = torch.tensor([0.70, 0.00, 1.03], device=self.device, dtype=torch.float32)
        init_goal_state[:, 2] = self.table_stand_height + 0.2
        init_goal_state[:, :2] = centered_goal_xy_state[0:2] + 0.5 * self.start_position_noise * \
                                        (torch.rand(self.num_envs, 2, device=self.device) - 0.25)
