...
class Ur5pickandplace(BaseTask):
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        barrier_height = 0.15
        barrier_pos = [0.30, 0.40, self.table_stand_height + barrier_height / 2]
        barrier_opts = gymapi.AssetOptions()
        barrier_opts.fix_base_link = True
        barrier_asset = self.gym.create_box(self.sim, *[0.7, 0.05, barrier_height], barrier_opts)
    ...
    def init_data(self):
        ...
        self.init_goal_pos[:,:3] = to_torch([0.30, 0.55, self.table_stand_height + self.cubeA_size / 2 + 0.5], device=self.device)
    ...
    def _random_cubeA_state(self, _init_cubeA_state, env_ids):
        ...
        centered_cube_xy_state = torch.tensor([0.30, 0.135, 0.83], device=self.device, dtype=torch.float32)
    ...
    def _random_goal_state(self, init_goal_state):
        centered_goal_xy_state = torch.tensor([0.30, 0.55, 1.36], device=self.device, dtype=torch.float32)
