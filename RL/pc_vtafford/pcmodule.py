...
class Network(nn.Module):
    ...
    def forward(self, batch):  
        ...
        whole_feats = self.pointnet2(data)  # B*128*N  BFN
...
if __name__ == '__main__':
    ...
    pct = torch.ones(2,10,4).uniform_(0,1).to(device) #each tactile for 5 points
    pct[:,:,3] = 1  #tactile labels

    pcv = torch.ones(2,20,4).uniform_(0,1).to(device) #visual for 20 points
    pcv[:,:,3] = 0

    concatenated = torch.cat((pct, pcv), dim=1)

    # 将第二个维度的元素打乱
    shuffled = concatenated[:, torch.randperm(concatenated.size(1)), :]
    ...
    #print(output)
    #print(pcs)
    #print(output.shape)  #32 2048
    # top_values, top_indices = torch.topk(output, 5, dim=1)
    # print(top_values)
    # print(top_indices)
    # tops = torch.gather(pcs, 1, top_indices.unsqueeze(-1).expand(-1, -1, pcs.size(-1)))
    # top_points = torch.cat((tops[:,:,0:3] , top_values.unsqueeze(-1)),dim=2).reshape(tops.size(0),-1)
    # print(top_points)
