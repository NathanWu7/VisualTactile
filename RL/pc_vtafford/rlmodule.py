...
class Student(nn.Module):
    def __init__(self, obs_shape,  latent_size, actions_shape, num_envs, device, model_cfg):
        ...
        self.pcs = torch.zeros(self.replay_size, num_envs, 8192, 4, device=device)
    ...
    def batch_sampler(self):
        ...
        data_pcs = self.pcs.reshape(-1,8192,4)
...
if __name__ == '__main__':
    ...
    predictions = torch.ones(4,7).uniform_(0,1).to(device) 
    labels = torch.ones(4,7).uniform_(0,1).to(device) 
    for i in range(3000):
